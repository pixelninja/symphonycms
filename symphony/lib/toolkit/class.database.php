...
class Database
{
     ...
     *
     * @link https://dev.mysql.com/doc/refman/5.7/en/query-cache.html
     */
    ...
    public function fetch($query = null, array $params = array(), array $values = array())
    {
        ...
        } elseif (is_null($this->_lastResult) || $this->_lastResult === false) {
            return array();
        }
    ...
    public function query($query, array $params = array(), array $values = array()) {
        ...
        } elseif ($query_type == self::__READ_OPERATION__ && !preg_match('/^\s*SELECT\s+SQL(_NO)?_CACHE/i', $query)) {
        ...
        if ($this->_result instanceof PDOStatement && $query_type == self::__READ_OPERATION__) {
            ...
            } elseif ($params['fetch-type'] == 'OBJECT') {
    ...
    private function q($query, $values, $close = true)
    {
        ...
        if ($this->conn->errorCode() !== PDO::ERR_NONE) {
            ...
            return false;
        } elseif ($this->_result instanceof PDOStatement) {
