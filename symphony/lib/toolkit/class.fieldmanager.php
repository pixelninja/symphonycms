...
class FieldManager implements FileResource
{
    ...
    {
        return Symphony::Database()->fetchVar('type', 0, "
            SELECT `type` FROM `tbl_fields` WHERE `id` = ? LIMIT 1",
            array($id)
        );
    }
    ...
    {
        return Symphony::Database()->fetchVar('element_name', 0, "
            SELECT `element_name` FROM `tbl_fields` WHERE `id` = ? LIMIT 1",
            array($id)
        );
    }
    ...
    {
        return (int)Symphony::Database()->fetchVar('count', 0, "
            SELECT COUNT(*) AS `count` FROM `tbl_fields` WHERE `type` = ?",
            array($field_type)
        ) > 0;
    }
